...
4. **Triathlon.html**
    ...
    * I was shown a warning saying: "Empty heading". This related to the same heading that had caused the above warning. However, this heading is deliberately empty and is populated with text by a function in the quiz.js code when the user clicks on the 'get my score' button. As this was a warning, not an error, I chose not to change my code.
...
* I was shown two errors that were both saying: "Value error: margin-top:0 is not a margin-top value". These related to the .membership-benefits and .membership-options classes. I removed the margin-top css code and this fixed the errors.
...
2. **Maps.js**
    * I was shown seven warnings:
        1. Four warnings related to 'Eversion 6'. I researched this and found this [post](https://stackoverflow.com/questions/37247474/es6-in-jshint-jshintrc-has-esversion-but-still-getting-warning-using-atom) giving this comment /*jshint esversion: 6 */ to add to the top of the javascript code to reswolve the warnings.
        2. Two warnings for missing semicolons. I added the semicolons to my code and this fixed the error.
        3. One warning saying: "functions declared within loops referencing an outer scoped loop may lead to confusing semantics". This related to the closeOtherInfo function and InfoObj array in the code for the info window pop up. The positioning of this code was the same in other examples I could find and trying to re-arrange my code didn't resolve the issue. This remains a current problem and is still presenting this warning. This is a learning point for me and I will look to resolve this in the future.
            * Whilst not warnings, JSHint also identified google as an undefined variable and initMap as an unused variable. These were taken directly from the [Google Maps API documentation](https://developers.google.com/maps/documentation/javascript/adding-a-google-map) used to initialise the map and I therefore chose not to change the code.
...
3. **Membership-packages.js**
    * I was shown seven warnings all saying: "missing semicolons". I had missed the semicolons from the end of each function. I added the seven semicolons and this fixed the warning.

        When I made the above changes, the validation passed.

4. **quiz.js**
    * I was shown two warnings related to 'Eversion 6'. Like with the map.js file, I added the comment /*jshint esversion: 6 */ to the top of my code and this fixed the error.
...
As a user, I tried to submit the form without completing all the form field inputs and received the following error message and the form wouldn't submit. This worked correctly as intended.
...
All the pages scored highly for accessibility and SEO. Four out of the five pages acheived a good score for performance too. However the report for my Training page was poor. Having read through the detail of the report, this low score is caused because the web page is very slow to load up. This is because the page contains a hero image and the large interactive map. The map in particular is causing the slow loading time. For a real project I would have to find alternative methods to overcome this issue so as to not impair the user experience of my website. In the future I will try compressing these features, to increase the speed at which the page loads and dramatically improve the Lighthouse score for this page.
...
The following points were found during testing that need to be rectified in the future to make the website work better for all users (inc. the club).

* When as a user I submitted an email address that doesn't actually exist, the form could still be submitted to the club and an email arrived in my (the club's) inbox. However, as the user confirmation email failed to be successfully delivered to the user's inbox (because it didn't exist) I received an email to let me know. 
